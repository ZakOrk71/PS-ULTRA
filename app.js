...
function App() {
  ...
  useEffect(() => {
    try {
      ...
      data.workoutHistory = workoutHistory;
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
    } catch (e) {
    }
  }, [workoutHistory]);
  ...
  const parseRest = (str) => {
    if (!str || str === "-" || str === "\u2014") return 90;
    const s = String(str).toLowerCase();
    const nums = s.match(/[\d.]+/g);
    if (!nums) return 90;
    const avg = nums.reduce((a, b) => a + parseFloat(b), 0) / nums.length;
    return s.includes("min") ? Math.round(avg * 60) : Math.round(avg);
  };
  const parseSets = (str) => {
    const m = String(str || "").match(/(\d+)\s*[×x]/);
    return m ? Math.min(8, Math.max(1, parseInt(m[1]))) : 3;
  };
  const startWorkout = (seanceId) => {
    setActiveWorkout({ seanceId, startTime: Date.now(), sets: {} });
    setShowHistory(false);
    if (typeof window !== "undefined") window.scrollTo({ top: 0, behavior: "smooth" });
  };
  const cancelWorkout = () => {
    if (typeof window === "undefined" || window.confirm("Abandonner la s\xE9ance en cours ? (rien ne sera enregistr\xE9)")) {
  ...
  const toggleSet = (exoIdx, setIdx, restStr) => {
    setActiveWorkout((aw) => {
      ...
      const sets = __spreadValues({}, aw.sets);
      if ((_a2 = sets[key]) == null ? void 0 : _a2.done) {
        delete sets[key];
      } else {
  ...
  const updateSetField = (exoIdx, setIdx, field, value) => {
    setActiveWorkout((aw) => {
      if (!aw) return aw;
      const key = `${exoIdx}-${setIdx}`;
      const sets = __spreadValues({}, aw.sets);
      sets[key] = __spreadProps(__spreadValues({}, sets[key] || { done: false }), { [field]: value });
      return __spreadProps(__spreadValues({}, aw), { sets });
    });
  };
  const finishWorkout = () => {
    if (!activeWorkout) return;
    const seance = getSeance(activeWorkout.seanceId);
    const doneSets = Object.entries(activeWorkout.sets).filter(([k, v]) => v.done);
    const entry = {
      id: Date.now() + "_" + Math.random().toString(36).slice(2, 6),
      date: (/* @__PURE__ */ new Date()).toISOString(),
      seanceId: activeWorkout.seanceId,
      label: seance ? seance.label : activeWorkout.seanceId,
      emoji: seance ? seance.emoji : "\u{1F4AA}",
      color: seance ? seance.color : "#00ff88",
      durationMin: Math.max(1, Math.round((Date.now() - activeWorkout.startTime) / 6e4)),
      setsCount: doneSets.length,
      details: doneSets.map(([k, v]) => {
        var _a2;
        const [ei] = k.split("-");
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', system-ui, sans-serif", paddingBottom: "calc(95px + env(safe-area-inset-bottom))" } }, /* @__PURE__ */ React.createElement("div", { style: { background: "var(--header-bg)", padding: "26px 18px 20px", position: "sticky", top: 0, zIndex: 10, paddingTop: "calc(26px + env(safe-area-inset-top))", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: "1px solid var(--border)" } }, /* @__PURE__ */ React.createElement("div", { style: { maxWidth: 680, margin: "0 auto" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("div", { style: {
  ...
  } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, letterSpacing: 2.5, color: "#666", textTransform: "uppercase", fontWeight: 700 } }, "PS Ultra \xB7 Progz"), /* @__PURE__ */ React.createElement("button", { className: "btn", onClick: () => { const t = theme === "dark" ? "light" : "dark"; setTheme(t); document.body.classList.toggle("light", t === "light"); try { localStorage.setItem("ps_ultra_theme", t); } catch(e) {} }, style: { background: theme === "light" ? "#e0e0ea" : "#1a1a24", border: "none", borderRadius: "50%", width: 28, height: 28, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", padding: 0, flexShrink: 0 } }, theme === "dark" ? "☀️" : "\u{1F319}")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 6, alignItems: "center" } }, /* @__PURE__ */ React.createElement("button", { className: "btn", title: "Rafra\xEEchir l'appli", onClick: () => { if ("serviceWorker" in navigator) { navigator.serviceWorker.getRegistrations().then(function(r) { r.forEach(function(sw) { sw.update(); }); }); } window.location.reload(true); }, style: { background: "#1a1a24", border: "none", borderRadius: "50%", width: 28, height: 28, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", padding: 0, flexShrink: 0 } }, "\u{1F504}"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 5, background: "#050508", border: "1px solid #14141e", borderRadius: 24, padding: 3 } }, [{ k: "nuit", l: "\u{1F319}", f: "Nuit" }, { k: "jour", l: "\u2600\uFE0F", f: "Jour" }].map((c) => /* @__PURE__ */ React.createElement("button", { key: c.k, onClick: () => applyCycle(c.k), style: {
  ...
  }))), tab === "S\xE9ances" && activeWorkout && (() => {
    const s = getSeance(activeWorkout.seanceId);
    if (!s) return null;
    const totalSets = s.exos.reduce((sum, ex) => sum + parseSets(ex.s), 0);
    const doneCount = Object.values(activeWorkout.sets).filter((v) => v.done).length;
    ...
    return /* @__PURE__ */ React.createElement("div", null, showRestoredBanner && /* @__PURE__ */ React.createElement("div", { style: { background: "#0a1a0f", border: "1px solid #1a4a2a", borderRadius: 12, padding: "11px 14px", marginBottom: 12, display: "flex", alignItems: "center", gap: 9 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 16 } }, "\u{1F4BE}"), /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, fontWeight: 700, color: "#4ade80" } }, "S\xE9ance retrouv\xE9e"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#888", marginTop: 1 } }, "Reprends o\xF9 tu t\'\xE9tais arr\xEAt\xE9(e)")), /* @__PURE__ */ React.createElement("button", { onClick: () => setShowRestoredBanner(false), style: { background: "none", border: "none", color: "#555", cursor: "pointer", fontSize: 16, padding: 4 } }, "\xD7")), /* @__PURE__ */ React.createElement("div", { style: { background: `linear-gradient(135deg, ${s.color}22, ${s.color}08)`, border: `1px solid ${s.color}44`, borderRadius: 16, padding: "18px", marginBottom: 16 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 11 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 28 } }, s.emoji), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 9, color: s.color, fontWeight: 800, letterSpacing: 2, textTransform: "uppercase" } }, "En cours"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 17, fontWeight: 800, color: "#fff" } }, s.label))), /* @__PURE__ */ React.createElement("button", { onClick: cancelWorkout, style: { background: "#1a1a24", border: "none", color: "#888", borderRadius: 10, padding: "8px 12px", cursor: "pointer", fontSize: 11, fontWeight: 700 } }, "Abandonner")), /* @__PURE__ */ React.createElement("div", { style: { height: 8, background: "#06060c", borderRadius: 4, overflow: "hidden", marginBottom: 8 } }, /* @__PURE__ */ React.createElement("div", { style: { height: "100%", width: pct + "%", background: s.color, borderRadius: 4, transition: "width 0.3s", boxShadow: `0 0 10px ${s.color}` } })), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", fontSize: 11, color: "#888", fontWeight: 600 } }, /* @__PURE__ */ React.createElement("span", null, doneCount, "/", totalSets, " s\xE9ries"), /* @__PURE__ */ React.createElement("span", null, "\u23F1 ", elapsedMin, " min"))), s.exos.map((ex, ei) => {
      ...
      const exoDone = Array.from({ length: nSets }).every((_, si) => {
        var _a2;
        return (_a2 = activeWorkout.sets[`${ei}-${si}`]) == null ? void 0 : _a2.done;
      });
      return /* @__PURE__ */ React.createElement("div", { key: ei, style: { background: exoDone ? `${s.color}0e` : "#0a0a12", border: `1px solid ${exoDone ? s.color + "44" : "#14141e"}`, borderRadius: 14, padding: "15px 16px", marginBottom: 11, transition: "all 0.2s" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 9, marginBottom: 11 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 26, height: 26, borderRadius: 7, background: s.color + "22", color: s.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, flexShrink: 0 } }, ei + 1), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 700, color: "#fff" } }, exoName), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#666", marginTop: 1 } }, "Objectif : ", ex.s, " \xB7 repos ", ex.rest)), exoDone && /* @__PURE__ */ React.createElement("span", { style: { color: s.color, fontSize: 18 } }, "\u2713")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 7 } }, Array.from({ length: nSets }).map((_, si) => {
        const setData = activeWorkout.sets[`${ei}-${si}`];
        const done = setData == null ? void 0 : setData.done;
        return /* @__PURE__ */ React.createElement("div", { key: si, style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => toggleSet(ei, si, ex.rest), style: {
          width: 34,
          height: 34,
          borderRadius: 9,
          flexShrink: 0,
          cursor: "pointer",
          background: done ? s.color : "#06060c",
          border: `1.5px solid ${done ? s.color : "#222"}`,
          color: done ? "#000" : "#444",
          fontSize: 14,
          fontWeight: 800,
          display: "flex",
        ...
        } }, done ? "\u2713" : si + 1), /* @__PURE__ */ React.createElement(
          "input",
          {
            type: "text",
            inputMode: "numeric",
            placeholder: "reps",
            value: (setData == null ? void 0 : setData.reps) || "",
            onChange: (e) => updateSetField(ei, si, "reps", e.target.value),
            style: { flex: 1, minWidth: 0, background: "#06060c", border: "1px solid #222", borderRadius: 9, padding: "9px 11px", color: "#fff", fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }
          }
          ...
          "input",
          {
            type: "text",
            inputMode: "numeric",
            placeholder: "kg (option)",
            value: (setData == null ? void 0 : setData.charge) || "",
            onChange: (e) => updateSetField(ei, si, "charge", e.target.value),
            style: { flex: 1, minWidth: 0, background: "#06060c", border: "1px solid #222", borderRadius: 9, padding: "9px 11px", color: "#fff", fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }
          }
        ));
      })));
    ...
    const lw = lastWorkoutFor(s.id);
    if (!lw) return null;
    const d = new Date(lw.date);
    return /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#666", textAlign: "center", marginBottom: 14, marginTop: -4 } }, "Derni\xE8re fois : ", d.toLocaleDateString("fr-FR", { day: "numeric", month: "short" }), " \xB7 ", lw.setsCount, " s\xE9ries \xB7 ", lw.durationMin, " min");
  })(), /* @__PURE__ */ React.createElement("div", { style: { background: "#000", borderRadius: 9, padding: "11px 13px", fontSize: 12, color: "#aaa", marginBottom: 14, lineHeight: 1.5 } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#a78bfa", fontWeight: 700, fontSize: 9, letterSpacing: 1.5, textTransform: "uppercase" } }, "\xC9chauff. \xB7 "), s.echauff), s.exos.map((ex, i) => {
    const activeKey = modes[modeIdx].key;
  ...
  }))))), restTimer && /* @__PURE__ */ React.createElement("div", { style: {
    position: "fixed",
  ...
  } }, /* @__PURE__ */ React.createElement("div", { style: { position: "relative", width: 44, height: 44, flexShrink: 0 } }, /* @__PURE__ */ React.createElement("svg", { width: "44", height: "44", style: { transform: "rotate(-90deg)" } }, /* @__PURE__ */ React.createElement("circle", { cx: "22", cy: "22", r: "19", fill: "none", stroke: "#0a2a1a", strokeWidth: "4" }), /* @__PURE__ */ React.createElement(
    "circle",
    {
      cx: "22",
      cy: "22",
    ...
    fontSize: 12,
    fontWeight: 700
  } }, "+15s"), /* @__PURE__ */ React.createElement("button", { onClick: () => setRestTimer(null), style: {
    background: "#00ff88",
    border: "none",
    color: "#000",
    borderRadius: 10,
